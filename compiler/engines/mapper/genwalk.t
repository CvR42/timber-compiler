...
.. Information is passed using the global context 'gc'
..
.set extra_par gc
.set extra_par_t global_context
..
...
.if ${eq $(type) $(nowtype)}
$(indent)$(var) = ($(type)) rewrite_$(type)_action( $(var), $(extra_par) );
.else
$(indent)$(var) = ($(type)) rewrite_$(type)_action( to_$(type)( $(var) ), $(extra_par) );
.endif
.endmacro
..
.. Given an indent 'indent', a variable 'var' to walk into, the type of
...
.if ${eq $(type) $(nowtype)}
$(indent)$(var) = ($(type)) rewrite_$(type)_walker( $(var), $(extra_par) );
.else
$(indent)$(var) = ($(type)) rewrite_$(type)_walker( to_$(type)( $(var) ), $(extra_par) );
.endif
.endmacro
..
...
.endmacro
..
.. And now some generic support for treewalking...
.insert gensupport.t
