/* $Id: FFF-p_Scheduling.C,v 1.1 2000/10/19 21:09:45 andi Exp $ */
...
#include "FFF-p_Scheduling.H"
   ...
   */
FFF_p_Scheduling::FFF_p_Scheduling (void)
    : Scheduling()
{
  ...
  {
    proc_ready_task[pid] = new PseudoHeap (proc->GetNrProc());
    proc_item[pid] = new PrioItem (pid, 0);
    proc_list->Enqueue (proc_item[pid]);
  }

  global_ready_task = new PseudoHeap (proc->GetNrProc());
  ready_task_by_blevel = new PseudoHeap (proc->GetNrProc());
  ready_task_by_blevel_item = new PPseudoPrioItem [dag->GetNrNodes()];
//  CP = 0;
...
//      CP = node->GetBLevel();
    ...
    if (node->GetNrInEdges() == 0)
    {
      ...
      ready_task_by_blevel_item[nid] = new PseudoPrioItem (nid, node->GetBLevel());
      ready_task_by_blevel->Enqueue (ready_task_by_blevel_item[nid]);
    }
  }
}

FFF_p_Scheduling::~FFF_p_Scheduling ()
{
  for (int pid = 0; pid < proc->GetNrProc(); pid++)
   ...
   */
void
FFF_p_Scheduling::Schedule (void)
{
...
PFLB_p_Node
FFF_p_Scheduling::ScheduleTask (void)
{
  ...
  PPseudoPrioItem task_item = (PPseudoPrioItem) ready_task_by_blevel->Head();
  PFLB_p_Node task = (PFLB_p_Node) dag->GetNode (task_item->GetIDX());
  ...
  PFLB_p_Node task1 = (PFLB_p_Node) proc_ready_task[pid]->Head();
  if (task1 == task)
    task1 = 0;
  PFLB_p_Node task2 = (PFLB_p_Node) global_ready_task->Head();
  if (task2 == task)
    task2 = 0;
  ...
  PPseudoPrioItem tmp_item = (PPseudoPrioItem) ready_task_by_blevel_item[task->GetID()];
...
void
FFF_p_Scheduling::UpdateReadyTaskLists (PFLB_p_Node task)
{
  for (unsigned int i = 0; i < task->GetNrOutEdges(); i++)
  {
    ...
    if (succ->DecNrUnschedPred() == 0)
    {
      ...
      ready_task_by_blevel_item[sid] = new PseudoPrioItem (sid, succ->GetBLevel());
