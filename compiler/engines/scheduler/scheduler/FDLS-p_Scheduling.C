/* $Id: FDLS-p_Scheduling.C,v 1.1 2000/10/19 21:09:45 andi Exp $ */
...
#include "FDLS-p_Scheduling.H"
   ...
   */
FDLS_p_Scheduling::FDLS_p_Scheduling (double pseudo_heap_size)
    : Scheduling()
{
  proc_list = new DirectHeap (proc->GetNrProc());
  BL_nonEP_task_list = new PseudoHeap (pseudo_heap_size);
  BL_nonEP_task_item = new PPseudoPrioItem [dag->GetNrNodes()];
  prio_nonEP_task_list = new PseudoHeap (pseudo_heap_size);
  ...
  {
    PFDLS_p_Node node = (PFDLS_p_Node) dag->GetNode (nid);
    node->ComputeBLevel();
    ...
    {
      BL_nonEP_task_item[nid] = new PseudoPrioItem (nid, node->GetBLevel());
...
FDLS_p_Scheduling::~FDLS_p_Scheduling ()
{
  delete proc_list;
...
void
FDLS_p_Scheduling::Schedule (void)
{
...
FDLS_p_Scheduling::ScheduleTask (void)
{
  ...
  if (ep_pid != NONE)
  {
    ...
    ep_task = (PFDLS_p_Node) dag->GetNode (nid);
    ep_prio = ep_task->ComputePrio (ep_pid);
    sched_time ep_prio2 = ep_task->ComputePrio (non_ep_pid);
  ...
  if (proc_EP_task_item[task->GetID()] != NULL)
  {
    ...
    delete proc_EP_task_list[p]->RemoveItem (proc_EP_task_item[task->GetID()]);
    proc_EP_task_item[task->GetID()] = NULL;

    PFDLS_p_Node t = (PFDLS_p_Node) proc_EP_task_list[p]->Head();
    PPrioItem item = active_proc_list->GetItem (p);
    if (t != NULL)
    {
      item->SetIDX (p);
  ...
  for (;;)
  {
    ...
    PFDLS_p_Node t = (PFDLS_p_Node) dag->GetNode (nid);
    if (t->GetLMT() < prt)
    {
      PPrioItem item = active_proc_list->GetItem (pid);
      item->SetIDX (pid);
  ...
  PFDLS_p_Node t = (PFDLS_p_Node) proc_EP_task_list[pid]->Head();
  if (t != NULL)
  {
    PPrioItem item = active_proc_list->GetItem (pid);
    item->SetIDX (pid);
  ...
  {
    PFDLS_p_Node succ = (PFDLS_p_Node) task->GetSuccNode (i);
    ...
    if (succ->DecNrUnschedPred() == 0)
    {
      ...
      BL_nonEP_task_item[sid] = new PseudoPrioItem (sid, succ->GetBLevel());
