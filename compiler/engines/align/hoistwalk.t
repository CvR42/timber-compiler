...
static $t rewrite_$t_walker($t $(var), OwnerExpr_list *o_l )
.endmacro
..
.. Given the name 'var' of the variable that holds the datastructure
...
.if ${eq $(type) $(nowtype)}
$(indent)$(var) = ($(type)) rewrite_$(type)_action( $(var), o_l );
.else
$(indent)$(var) = ($(type)) rewrite_$(type)_action( to_$(type)( $(var) ), o_l );
.endif
.endmacro
..
.. Given an indent 'indent', a variable 'var' to walk into, the type of
...
.if ${eq $(type) $(nowtype)}
$(indent)$(var) = ($(type)) rewrite_$(type)_walker( $(var), o_l );
.else
$(indent)$(var) = ($(type)) rewrite_$(type)_walker( to_$(type)( $(var) ), o_l );
.endif
.endmacro
..
