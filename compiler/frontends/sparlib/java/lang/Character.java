...
class CharacterProperties {
	...
	new CharacterProperties((char)0x0000, Character.CONTROL, false, (short)-1,
    ...
    static {
	// This is going to fail for the first character property
	// that is really used...
    }
     ...
     */
    static CharacterProperties decodeProp(char ch, int index) {
	byte tbl[] = propTable;
	...
	if (method < 3) {
		...
		generic += (char)(0xFFFF & ((int)ch - (int)rangeStart[index]));
	    }
...
public final class Character implements Serializable {
  ...
  {
      // CvR: this is only partially right: it only works for plain
      // ASCII. Anything more special is currently implemented
      // incorrectly.
  ...
  {
      // TODO: this is a quick hack to get the stuff working for
      // the ASCII character set. For Unicode more subtility is needed.
      if( ch>='0' && ch<='9' ){
	  return (int) ch - (int) '0';
      }
      if( ch>='a' && ch<='z' ){
