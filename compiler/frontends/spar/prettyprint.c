...
static void pp_expression( FILE *f, const_TypeEntry_list typelist, const_Entry_list symtab, const_expression x )
{
	...
	{
	    // TODO: print in a more readable format when possible
	    vnus_long n = vnus_doublebits_to_long( to_const_DoubleExpression(x)->v );
	...
	case TAGNewArrayExpression:
	{
		...
		if( is_zero( to_OptExpr(init)->x, NULL ) == AF_YES ){
		    nulled = true;
		    fputs( "nullednew", f );
		}
		else {
...
)
{
    ...
    *first = false;
    ...
    fputs( "] pointer record ", f );
    pp_tmsymbol( f, e->recordname );
    fputs( " pragma [inline,nullvalue=false] statements ", f );
    pp_tmsymbol( f, scopenm );
    fputs( " [\n", f );
    pp_indent( f, 2*INDENTSTEP );
    fputs( "assign field *", f );
...
)
{
    ...
    pp_indent_line( f, INDENTSTEP, "globalvariable __gc_reflink_chain pointer record __gc_reflink cast pointer record __gc_reflink null," );
